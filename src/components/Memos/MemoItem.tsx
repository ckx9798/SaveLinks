...
export default function MemoItem({ memo, setMemos, selectedMemo, setSelectedMemo }: MemoItemProps) {
  ...
  return (
    <>
      <div
        className="relative mt-2 h-auto w-full overflow-hidden rounded-xl border-2 border-gray01 bg-gray01 shadow-xl hover:scale-105 lg:p-3"
        onClick={() => {
      ...
      >
        <div className="flex h-full flex-col justify-between px-2 pb-3">
          ...
          <h2 className="line-clamp-1 min-h-[72px] text-xl leading-6 text-gray04 md:mb-3 md:min-h-[90px] md:text-2xl md:font-medium">
            {memo.description || `No description provided`}
          </h2>
          <p className="-mb-2 hidden text-lg text-gray04 md:block md:text-xl">
